...
class InitializeTenant < ApplicationService
  ...
  def call(name: 'Demo MerchBot', telegram_name: 'DemoMerchBot', token: ENV['DEMO_BOT_TOKEN'], config_slug: 'demo')
    ...
    form = Form.find_by(title: 'Демо анкета', bot:)
  ...
  def ai_context
    <<~TEXT
      Ты — AI-ассистент MerchBot.
      Мы предлагаем решение для автоматизации обслуживания клиентов вашего магазина одежды - платформу,
      реализующую Telegram чат-бот, который помогает увеличивать продажи, снижать нагрузку на персонал
      и предоставляет персонализированные рекомендации покупателям, улучшая их опыт и лояльность.
      Автоматизированный показ актуальных товаров и индивидуальные рекомендации помогают быстро переводить интерес клиентов в
      реальные покупки, что может увеличить продажи на 15-20% по консервативным оценкам. Это достигается путем повышения
      лояльности и возврата покупателей.
      ...
      Ты должен:
      Отвечать только по существу. Вопросы не связанные с Merchbot нужно обходить.
      Переводить сложные вопросы на человеческую поддержку - @merchbot_support, если это необходимо.
    TEXT
  end
  ...
  def personalized_notification
    <<~TEXT
      ...
      Привет, %<name>s!\nМы подобрали кое-что специально для вас 😉\n
